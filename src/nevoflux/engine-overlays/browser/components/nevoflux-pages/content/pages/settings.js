...
const Settings = {
  ...
  async _showProviderModal(provider) {
    ...
    try {
      ...
      if (data) {
        if (data.has_api_key) {
          document.getElementById("llm-modal-apikey").placeholder = `Current: ${data.api_key || "configured"}`;
        }
        if (data.model) {
        ...
        document.getElementById("llm-modal-key-help").textContent =
          data.has_api_key ? "Key is configured. Leave blank to keep current." : "Required for this provider.";
      }
    } catch (e) {
  ...
  _ensureMcpModal() {
    ...
    // Server Name
    form.appendChild(this._createModalFormGroup(
      "Server Name *", "mcp-modal-name", "text",
      "e.g., my-server", "A unique identifier for this server"
    ));
    ...
    stdioFields.appendChild(this._createModalFormGroup(
      "Command *", "mcp-modal-command", "text",
      "npx -y @modelcontextprotocol/server-name", "Command to start the server process"
    ));
    stdioFields.appendChild(this._createModalFormGroup(
      "Arguments", "mcp-modal-args", "text",
      "--arg1 value1 --arg2 value2", "Space-separated command arguments"
    ));
    const stdioRow = document.createElement("div");
    stdioRow.className = "mcp-form-row";
    const workDirGroup = this._createModalFormGroup(
      "Working Directory", "mcp-modal-workdir", "text", "/path/to/dir"
    );
    const envGroup = this._createModalFormGroup(
      "Environment", "mcp-modal-env", "text", "KEY=value,KEY2=value2"
    );
    ...
    sseFields.appendChild(this._createModalFormGroup(
      "SSE Endpoint URL *", "mcp-modal-sse-url", "url",
      "http://localhost:3000/events", "Server-Sent Events endpoint URL"
    ));
    const sseRow = document.createElement("div");
    sseRow.className = "mcp-form-row";
    sseRow.appendChild(this._createModalFormGroup(
      "Reconnect (ms)", "mcp-modal-sse-reconnect", "number", "5000"
    ));
    sseRow.appendChild(this._createModalFormGroup(
      "Timeout (ms)", "mcp-modal-sse-timeout", "number", "30000"
    ));
    sseFields.appendChild(sseRow);
    sseFields.appendChild(this._createModalFormGroup(
      "Headers (JSON)", "mcp-modal-sse-headers", "text",
    ...
    httpFields.appendChild(this._createModalFormGroup(
      "Base URL *", "mcp-modal-http-url", "url",
      "https://api.example.com/mcp", "Base URL for the HTTP API"
    ));
    ...
    httpRow.appendChild(this._createModalFormGroup(
      "Timeout (ms)", "mcp-modal-http-timeout", "number", "30000"
    ));
    httpFields.appendChild(httpRow);
    httpFields.appendChild(this._createModalFormGroup(
      "Headers (JSON)", "mcp-modal-http-headers", "text",
      '{"Content-Type": "application/json"}'
    ));
    httpFields.appendChild(this._createModalFormGroup(
      "API Key", "mcp-modal-http-apikey", "password", "Enter API key"
    ));
    form.appendChild(httpFields);

    // Description
    form.appendChild(this._createModalFormGroup(
      "Description", "mcp-modal-desc", "text",
  ...
  _openMcpModal(server, index) {
    ...
    // Reset all fields
    const ids = [
      "mcp-modal-name", "mcp-modal-command", "mcp-modal-args",
      "mcp-modal-workdir", "mcp-modal-env", "mcp-modal-sse-url",
      "mcp-modal-sse-reconnect", "mcp-modal-sse-timeout",
      "mcp-modal-sse-headers", "mcp-modal-http-url",
      "mcp-modal-http-timeout", "mcp-modal-http-headers",
      "mcp-modal-http-apikey", "mcp-modal-desc",
    ];
    for (const id of ids) {
  ...
  async _saveMcpServer() {
    ...
    if (type === "stdio") {
      ...
      if (headersStr) {
        try { server.headers = JSON.parse(headersStr); } catch { /* ignore parse error */ }
      }
    } else if (type === "http") {
  ...
  async _saveMdFiles() {
    ...
    if (errors.length) {
      ...
      if (status) {
        ...
        setTimeout(() => { status.className = "save-status"; }, 1500);
      }
    }
    this._checkMdDirty();
  },
