 ...
 *
 * This module wraps the Turndown library for use in Firefox system modules.
 */
...
var TurndownService = (function () {
  ...
  function collapseWhitespace(options) {
    ...
    while (node !== element) {
      if (node.nodeType === 3 || node.nodeType === 4) {
        ...
        if (isBlockFn(node) || node.nodeName === 'BR') {
          if (prevText) {
            prevText.data = prevText.data.replace(/ $/, '');
          }
          prevText = null;
          keepLeadingWs = false;
        } else if (isVoidFn(node) || isPre(node)) {
          prevText = null;
          keepLeadingWs = true;
        } else if (prevText) {
          keepLeadingWs = false;
  ...
  function RootNode(input, options, doc) {
    ...
    if (typeof input === 'string') {
      ...
      root = parsedDoc.getElementById('turndown-root');
    } else {
  ...
  TurndownService.prototype = {
    /**
     * Set the document context for parsing HTML strings
     */
...
/**
 * GFM Tables Plugin for Turndown
 * Based on turndown-plugin-gfm
...
function gfmTables(turndownService) {
  ...
  function cell(content, node) {
    ...
    var prefix = ' ';
    if (index === 0) prefix = '| ';
    // Remove newlines and pipes in cell content
    content = content.replace(/\n/g, ' ').replace(/\|/g, '\\|');
    return prefix + content + ' |';
  }

  // Helper: check if row is a heading row
  ...
  // Helper: check if node is first tbody
  function isFirstTbody(node) {
...
/**
 * GFM Strikethrough Plugin
 */
function gfmStrikethrough(turndownService) {
...
/**
 * Combined GFM Plugin (tables + strikethrough)
 */
function gfm(turndownService) {
  gfmTables(turndownService);
  gfmStrikethrough(turndownService);
}

// Export for Firefox ES modules
export { TurndownService, gfm, gfmTables, gfmStrikethrough };
