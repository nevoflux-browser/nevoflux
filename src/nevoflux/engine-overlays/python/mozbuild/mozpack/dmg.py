...
def rsync(source: Path, dest: Path):
    ...
    if source.is_dir():
        raw_source = str(source) + "/"
    # Tolerate rsync exit code 23 (partial transfer due to vanished/missing files).
    # This commonly happens when extracting DMGs on macOS where metadata files
    # (.DS_Store, .background, .VolumeIcon.icns) may not be fully accessible.
    result = subprocess.run(["rsync", "-a", "--copy-unsafe-links", raw_source, str(dest)])
